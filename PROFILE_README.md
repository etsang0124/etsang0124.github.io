...
│                             SKYLOFT PLATFORM                                │
├───────────────────────────────┬─────────────────────────────────────────────┤
│ 📱 Mobile & Web App           │ ⚡ Flask Core & Security                    │
│ • React Native 0.81 + Expo 54 │ • 3-Tier Model (Property → Unit → Listing)  │
│ • Reanimated 4 + Liquid Glass │ • Zero-Trust KYC Storage (/private/ vs /media)│
│ • 3-Question Onboarding & Feed│ • Strict RBAC/IAM Actor Context             │
├───────────────────────────────┼─────────────────────────────────────────────┤
│ 🎯 RecSys & Slate Ranker      │ 🕷️ Staging Pipeline                         │
│ • Audit-ready fit_score()     │ • Automated NYC luxury site crawler         │
│ • Commute travel decay curves │ • HTML AST parsing & schema extraction      │
│ • Empirical Beta cohort priors│ • Evidence-backed staging into mining.db    │
└───────────────────────────────┴─────────────────────────────────────────────┘
```

#### 🔑 Key Engineering Highlights:
- **Unified Native & Web Build**: Compiled via React Native Web and Expo SDK 54 into a single, responsive cross-platform runtime that operates seamlessly on iOS (Expo Go) and browsers.
- **Physical Media Isolation**: Public listing photography in `/media/` auto-strips EXIF and transcodes HEIC $\to$ JPEG, while unindexed KYC files (passports, pay stubs) reside strictly under `/private/` guarded by RBAC sessions.
- **Auditable Multi-Objective Scoring**: Ranks homes based on budget tolerance decay, MTA commute matrices to renter anchor locations, and behavioral engagement priors.
...
|---|---|
| **Languages** | `TypeScript`, `Python 3`, `JavaScript (ESNext)`, `SQL`, `HTML5/CSS3`, `Bash/Zsh` |
| **Mobile & Frontend** | `React Native`, `Expo (SDK 54)`, `React 19`, `Expo Router`, `Reanimated 4`, `React Native Web` |
| **Backend & APIs** | `Flask`, `Django`, `RESTful APIs`, `IAM & RBAC`, `Node.js`, `Pillow (Image Processing)` |
| **Data & Infrastructure** | `SQLite`, `PostgreSQL`, `Redis`, `Git / GitHub Actions`, `Docker`, `AWS S3` |
